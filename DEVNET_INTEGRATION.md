...
| The rebuild | **pushed** — `origin/master` and `feat/price-discovery-and-cip56` point at the same commit |
...
| Verification of the rebuild | **Daml Script scenarios** (92, all green), plus `./gradlew clean build` on both backends and a clean `tsc`. **No live-participant run of the auction.** |
