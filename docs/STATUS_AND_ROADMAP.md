...
| 22 | ✅ **Basket vaults on three chains (24 Sep)** — `evm-vault/`, 67 tests. Deployed AND exercised end to end on **Arc Testnet**, **Base Sepolia** and **Robinhood Chain Testnet**: two mock constituents, the vault, a 2-of-3 committee, create, redeem, a signed NAV, and two negatives reverting on-chain (`InsufficientSignatures`, `TierNotAccepted`). Every posted NAV carries a `fixingRef` back to the Canton record it projects. Records in `evm-vault/deployments/*.json`. Arbitrum Sepolia configured, not deployed | — | done |
| 23 | **Solana vault** — `solana-vault/`, Anchor. **Compiles; 82 tests pass, 0 fail.** The old "dependency graph" diagnosis was wrong: platform-tools v1.43 was a partial extraction missing the whole Rust sysroot; `agave-install init 2.2.12` fixes it. Devnet deploy written and funded (5.0 SOL against 2.54 of rent) but **not run** — the WSL VM wedges on network I/O and only `wsl --shutdown` clears it, which agents may not run. A committee of 3+ must pack signatures into ONE Ed25519 instruction (1232-byte limit) | user runs `wsl --shutdown` | hours |
| 24 | **The other EVM chains** — where tokenised equities actually live (`BUSINESS/1-LEARN/16-`): BNB Chain (Binance bStocks, withdrawable, $1.1B), Ethereum, Avalanche, Arbitrum (Bitget/Reality, withdrawal-locked behind a 3-of-5 Safe), Ink, Mantle, Morph. All EVM, so each is a config entry plus a faucet claim, not a build. Gas, not code, is the constraint | 22 | days |
| 25 | **All five seats real on DevNet** — Custodian and TransferAgent parties do NOT exist on the ledger, so two of the six advertised seats can never be filled. Allocate them, add to `LEDGER_PARTIES`, then prove all five confirm a real fixing with real evidence. Also decide the scheduler: `SCHEDULER_ENABLED=false`, so nothing proposes automatically and a pilot needs a human per strike | — | hours |
