...
| §2c venue range enforcement | **implemented, tested** — `ConfirmWithChecks` refuses a price outside the range, an inverted range, or a half-specified one (`testSignerProtocolEvidence`). The portal path for the venue is `evidence: {low, high}` for a range; **since 24 Sep 2026 a venue's `no-prints-attested` block is verified on both routes** (`SignerEvidence.verifiable`): the proposal must sit between the quoted bid and ask, and naming the condition without its quotes is refused as a bare tick. Before that a portal no-prints confirm reached the ledger with its quotes unchecked (`ConfirmWithChecksSeatTest`, `ProposalServiceSeatsTest`) |
...
| §4 reference checker implementations | **built — `signer-service` 0.2.0 (24 Sep 2026)** runs every seat (issuer under each reserve model, lender, venue, custodian, transfer-agent): it fetches `GET /api/signer-protocol?instrument=` per proposal and evaluates exactly that role's condition list, confirms `no-prints-attested` with bid/ask when the tape is empty (never alongside a range), halts rather than confirms on anything it cannot evaluate, treats a non-JSON 2xx as a failure, takes tolerances from config only, and redacts secrets from every log line. 64 tests; a quickstart per seat in its README. Each signer still points it at its own systems |
...
| **v2** reserve model per instrument | **implemented, deployed** — `signer.reserve-models` (`RESERVE_MODEL_CBTC`, `RESERVE_MODEL_CETH` env) → `SignerProtocol.reserveModelOf(instrument)`; the confirm path refuses a condition that does not belong to the seat *under that model*. Since 24 Sep 2026 the portal fetches `?instrument=` per proposal, so the guide panel and the confirm card show the model-narrowed list; ⚠️ the 422 body's `schema` still comes from the strict profile (audit medium #5, cosmetic) |
...
| **v2** identity | **deployed** — hosted host runs `AUTH_MODE=firebase`; only e-mail-verified Firebase users map to a roster row; request paths are normalised before the filter classifies them; **roster-only since 23 Sep 2026** — a verified identity that is not on the roster is 403 everywhere, including `/api/me` |
| **v2** portal evidence entry, every seat | **implemented 24 Sep 2026** — `ProposalCard` renders one input per evidence field of each ticked condition from `GET /api/signer-protocol?instrument=`, disables Confirm until every field is present (naming the missing one), sends `{checks, evidence: {condition: {field: value}}}`, shows a 422 verbatim, and gives the venue a "no prints in the window" toggle that swaps the range for bid/ask. Before this a manual confirm from an issuer, lender, custodian or transfer-agent was refused as a bare tick because the card never collected the numbers. The collapsible "Your seat" panel (`SeatGuide.tsx`) explains the seat, its conditions, the refuse path, the checker quickstart and the trust ladder from the same API data |
