 ...
 *
 * <p>Submissions use {@code submitAndWaitForTransaction} so that, on success,
...
public class LedgerService {
     ...
     * bridge between a failure seen here and the explanation held there. That matters
     * more on THIS build than on the local one: the devnet participant is operated by
     * somebody else, so the log with the answer in it is somebody else's log.
     */
    public Transaction submit(String actAs, HasCommands command) {
        ...
        if (connection.hasActiveToken()) {
            ...
            // It goes into gRPC call metadata and is never part of the command, so
            // nothing logged below can contain it.
        ...
        try {
            ...
            // for a flat Transaction with the LEDGER_EFFECTS shape, which still carries a
            // CreatedEvent for every contract the update creates (CreatedEvent implements
            // both Event and TreeEvent, so the decode below is unchanged).
     ...
     * that look identical from the UI and have nothing else in common. On the hosted
     * demo it also dates the last call against the CURRENT token, which is the fastest
     * way to see whether a refresh has quietly stopped happening.
     */
     ...
     * delta), which is what {@link #firstCreatedOf}/{@link #createdHoldingsOf}/{@link #batchOf}
     * read to recover the freshly-created contract ids.
     */
    ...
    public List<PartyView> listParties() {
        // DEVNET: the 3.x bindings drop the party-management admin service, and the
        // shared node's parties are pre-allocated + fixed — so read the configured
        // roster (ledger.parties = "Label=partyId,Label=partyId,...") instead of
        // querying the ledger. Same PartyView shape the rest of the desk expects.
        String cfg = connection.properties().getParties();
        ...
        if (cfg != null && !cfg.isBlank()) {
            for (String pair : cfg.split(",")) {
                ...
                out.add(new PartyView(party, label, label, true));
            }
        }
        return out;
    }
